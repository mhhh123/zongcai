...
if ( ! class_exists( 'CSF_Options' ) ) {
  class CSF_Options extends CSF_Abstract {
    ...
    public $args         = array(
      ...
      // framework title
      'framework_title'         => 'CeoMax-Pro主题 <small>By: 总裁主题(ceotheme.com)</small>',
      'framework_class'         => '',
      ...
      // footer
      'footer_text'             => '感谢使用 <a href="https://www.ceotheme.com/" target="_blank">CeoMax-Pro主题</a> 进行创作  │ Theme By 总裁主题 CeoTheme.com',
      'footer_after'            => '',
      'footer_credit'           => '感谢使用 <a href="https://www.ceotheme.com/" target="_blank">CeoMax-Pro主题</a> 进行创作  │ Theme By 总裁主题 CeoTheme.com',
      ...
      echo '<div class="csf csf-options'. esc_attr( $theme . $class . $wrapper_class ) .'" data-slug="'. esc_attr( $this->args['menu_slug'] ) .'" data-unique="'. esc_attr( $this->unique ) .'">';
        ...
        echo '<div class="csf-header-inner">';
          ...
          echo '<div class="csf-header-right">';
            ...
            echo ( $this->args['show_form_warning'] ) ? '<div class="csf-form-result csf-form-warning">'. esc_html__( 'CeoMax-Pro主题设置已更改，请保存！', 'csf' ) .'</div>' : '';

            echo ( $has_nav && $this->args['show_all_options'] ) ? '<div class="csf-expand-all" title="'. esc_html__( '显示所有设置', 'csf' ) .'"><i class="fas fa-outdent"></i></div>' : '';

            echo ( $this->args['show_search'] ) ? '<div class="csf-search"><input type="text" name="csf-search" placeholder="'. esc_html__( '搜索设置...', 'csf' ) .'" autocomplete="off" /></div>' : '';

            echo '<div class="csf-buttons">';
            echo '<input type="submit" name="'. esc_attr( $this->unique ) .'[_nonce][save]" class="button button-primary csf-top-save csf-save'. esc_attr( $ajax_class ) .'" value="'. esc_html__( 'Save', 'csf' ) .'" data-save="'. esc_html__( '保存中...', 'csf' ) .'">';
            echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( '重置当前分区', 'csf' ) .'" data-confirm="'. esc_html__( '您确定要重置此分区选项吗？重置后将清除此分区设置为默认值！建议先备份数据库！', 'csf' ) .'">' : '';
            echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. ( ( $this->args['show_reset_section'] ) ? esc_html__( '重置全部', 'csf' ) : esc_html__( 'Reset', 'csf' ) ) .'" data-confirm="'. esc_html__( '您确定要重置所有设置吗？重置后将清除所有设置为默认值！建议先备份数据库！', 'csf' ) .'">' : '';
            echo '</div>';
        ...
        if ( ! empty( $this->args['show_footer'] ) ) {
          ...
          echo '<div class="csf-buttons">';
          echo '<input type="submit" name="csf_transient[save]" class="button button-primary csf-save'. esc_attr( $ajax_class ) .'" value="'. esc_html__( '保存', 'csf' ) .'" data-save="'. esc_html__( '保存中...', 'csf' ) .'">';
          echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( '重置当前分区', 'csf' ) .'" data-confirm="'. esc_html__( '您确定要重置此分区选项吗？重置后将清除此分区设置为默认值！建议先备份数据库', 'csf' ) .'">' : '';
          echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. ( ( $this->args['show_reset_section'] ) ? esc_html__( '重置全部', 'csf' ) : esc_html__( 'Reset', 'csf' ) ) .'" data-confirm="'. esc_html__( '您确定要重置所有设置吗？重置后将清除所有设置为默认值！建议先备份数据库', 'csf' ) .'">' : '';
          echo '</div>';
