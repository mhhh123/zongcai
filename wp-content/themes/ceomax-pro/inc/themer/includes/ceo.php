...
class CEO {
    ...
    public static function panel_script(){
        ...
        wp_enqueue_script('vue', 'https://cdn.jsdelivr.net/npm/vue@2.6.12/dist/vue.min.js', array(), FRAMEWORK_VERSION, true);
        ...
        if(isset($options['ceofont']) && $options['ceofont']) wp_enqueue_script('ceofont');
    ...
    public static function editor_settings($args = array()){
        return array(
            ...
            'tinymce'       => array(
                'height'        => 150,
                'toolbar1' => 'formatselect,fontsizeselect,bold,italic,blockquote,forecolor,alignleft,aligncenter,alignright,link,bullist,numlist,ceoimg,ceotext',
                'toolbar2' => '',
                'toolbar3' => '',
                ...
                'external_plugins' => "{ceoimg: '".FRAMEWORK_URI."/assets/js/tinymce-img.js', ceotext: '".FRAMEWORK_URI."/assets/js/tinymce-text.js'}"
            )
        );
    }
    ...
    public static function _icons(){
        ...
        if( current_user_can( 'publish_posts' ) ){
            ...
            if(isset($options['ceofont']) && $options['ceofont'])
                $icons['if'] = array('name' => 'ceofont', 'icons' => json_decode(get_option('ceo_ceofont')));
    ...
    public static function update_icons($res, $options, $old_options){
        if($res['errcode']==0){
            ...
            $if = trim(isset($options['ceofont']) ? $options['ceofont'] : '');
            $old_if = trim(isset($old_options['ceofont']) ? $old_options['ceofont'] : '');
            if($if!=$old_if){
                $res['icon'] = 1;
                if($if && preg_match('/^(\/\/|http:\/\/|https:\/\/)/i', $if)){
                    ...
                    if(!is_wp_error($get) && $get['body']) {
                        ...
                        if($matches && isset($matches[1])){
                            update_option('ceo_ceofont', json_encode($matches[1]));
                        }
                    }
                }
            }
    ...
    public static function thumbnail( $url, $width = null, $height = null, $crop = false, $img_id = 0, $size = '', $single = false, $upscale = true ) {
        ...
        $aq_resize = CEO_Resize::getInstance();
    ...
    public static function thumbnail_url($post_id='', $size='full', $local=false){
        ...
        if( !$img ){
            ...
            if($img) {
                $image_sizes = apply_filters('ceo_image_sizes', array());
    ...
    public static function thumbnail_html($html, $post_id, $post_thumbnail_id, $size){
        ...
        $img_url = apply_filters('ceo_thumbnail_url', $img_url, $post_id, $post_thumbnail_id, $size);
        if($img_url) {
            $image_sizes = apply_filters('ceo_image_sizes', array());
    ...
    public static function check_post_images( $new_status, $old_status, $_post ){
        global $ceo_panel, $post;
        if( $ceo_panel && $ceo_panel->get_demo_config() ) {
            global $options, $wpdb;
    ...
    public static function reg_module( $module ){
        add_action('ceo_modules_'.$module, 'ceo_modules_'.$module, 10, 2);
        add_filter('ceo_modules', 'ceo_'.$module);
    }
    ...
    public static function icon($name, $echo = true, $class='', $alt='icon', $attr=''){
        if(preg_match('/^mti:/i', $name)){
            ...
            $str = '<i class="ceo-icon'.($class?' '.$class:'').'"><svg aria-hidden="true"><use xlink:href="#icon-'.$name.'"></use></svg></i>';
        }else if(preg_match('/^fa:/i', $name)){
            $name = preg_replace('/^fa:/i', '', $name);
            $str = '<i class="fa fa-'.$name.($class?' '.$class:'').'"></i>';
        }else if (preg_match('/^(\/\/|http:\/\/|https:\/\/)/i', $name)) {
            $str = '<i class="ceo-icon'.($class?' '.$class:'').'"><img class="j-lazy" src="' . esc_url($name) . '" alt="' . esc_attr($alt) . '" /></i>';
        }else{
            $str = '<i class="ceofont ceoicon-add-line"></i>';
        }
        if($echo) {
            echo $str;
        } else {
    ...
    public static function meta_filter( $res, $object_id, $meta_key, $single){
        $key = preg_replace('/^ceo_/i', '', $meta_key);
        if ( $key !== $meta_key ) {
            $filter = current_filter();
            $metas_key = '_ceo_metas';
            if( $filter=='get_post_metadata' ){
                $meta_type = 'post';
            }else if( $filter=='get_user_metadata' ){
                global $wpdb;
                $metas_key = $wpdb->get_blog_prefix() . '_ceo_metas';
                $meta_type = 'user';
            ...
            // 排除字段直接读取
            $exclude = apply_filters("ceo_exclude_{$meta_type}_metas", array());
            ...
            if( $filter=='get_term_metadata' && $metas=='' ) {
                ...
                if( $metas!='' ){
                    update_term_meta( $object_id, '_ceo_metas', $metas );
                }
            }
    ...
    public static function add_metadata($check, $object_id, $meta_key, $meta_value){
        ...
        if ( $key !== $meta_key ) {
            ...
            if( $filter=='add_post_metadata' || $filter=='update_post_metadata' ){
                ...
                $pre_key = $wpdb->get_blog_prefix() . '_ceo_metas';
                $meta_type = 'user';
            }
            ...
            if( $result && $meta_value != $pre_value && ($filter=='add_user_metadata' || $filter=='update_user_metadata') ) {
                do_action( 'ceo_user_meta_updated', $object_id, $meta_key, $meta_value, $pre_value );
            }
