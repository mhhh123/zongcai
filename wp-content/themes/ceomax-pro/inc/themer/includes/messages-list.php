...
class CEO_Messages_List extends WP_List_Table {
    protected $table;
    public function __construct(){
        global $wpdb;
        $this->table = $wpdb->prefix . 'ceo_messages';
        parent::__construct([
            'screen' => 'messages'
        ]);
    }
    ...
    public function column_status( $message ) {
        echo $message->status ? __('已读', 'ceo') : __('未读', 'ceo');
    }
    protected function handle_row_actions( $message, $column_name, $primary ) {
        ...
        $actions['delete'] = sprintf(
            ...
            wp_nonce_url( "?page=ceo-messages&action=delete-message&id=$message->ID", 'delete-message_' . $message->ID ),
            esc_js( sprintf( __( "You are about to delete this message\n  'Cancel' to stop, 'OK' to delete.", 'ceo' ), $message->ID ) ),
            __( 'Delete' )
        );
