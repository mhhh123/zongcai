...
if ($openid && $userInfo) {
    ...
    if (isset($user_exist) && (int) $user_exist > 0) {
        ...
        $login_name = _ceo('weixin_reg_prefix', 'CEO') . mt_rand(1000, 9999) . mt_rand(1000, 9999);
        $user_pass  = wp_create_nonce(rand(10, 1000));
        $nickname   = $metaInfo['name'];
        $userdata   = array(
            'user_login'   => $login_name,
            'user_email'   => $login_name._ceo('weixin_reg_suffix', '@ceo.com'),
