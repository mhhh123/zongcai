...
<div class="card-item b-r-4 ceo-background-default ceo-overflow-hidden ceo-vip-icons">
    ...
    //tag列表页 或搜索页
    $is_tag_list_page=is_archive() && get_query_var('tag');
    if($is_tag_list_page || is_search()){
        $category_arrs = get_the_terms(get_the_ID(), 'category');
        if ( ! empty($category_arrs[0])) {
            $term_id = $category_arrs[0]->term_id;
            $thumbnail_custom = get_term_meta($term_id, 'thumbnail_px_custom', 'true');
            if(!empty($thumbnail_custom) && !empty($thumbnail_custom['height'])){
                $img_w = ! empty($thumbnail_custom['width']) ? $thumbnail_custom['width'] : '';
                $img_h = ! empty($thumbnail_custom['height']) ? $thumbnail_custom['height'] : '';
                $style_w_h = 'height:'.$img_h.'px';
    ...
    <div class="ceo_app_img">
    	<a href="<?php the_permalink(); ?>" <?php echo _target_blank();?> class="cover ceo-display-block ceo-overflow-hidden <?php if($post_has_video)echo 'post-has-video ' ?>">
            ...
            ?>
            <img data-src="<?php echo post_thumbnail_src($img_w,$img_h); ?>" alt="<?php the_title(); ?>" src="<?php echo get_template_directory_uri().'/static/images/thumb-ing.gif'; ?>" class="ceo-width-1-1@s lazyload" style="height: <?php if($is_tag_list_page){echo $img_h.'px';} ?>">
    	</a>
	</div>
    <?php
