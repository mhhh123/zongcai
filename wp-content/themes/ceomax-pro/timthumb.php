...
/*
 * --- CeoTheme TimThumb ---
 * 1：如果开启TimThumb裁剪时，图片是外部链接 请在这里添加外链域名白名单，如：img.ceotheme.com 本地域名无需添加白名单
 * 2：如果开启TimThumb裁剪时，若未添加外链白名单，由于安全原因将无法显示缩略图
 * 3：如果已经使用了阿里云oss裁剪或其他裁剪模式，那么可以删除此文件
*/
...
//		if(! preg_match('/^image\/(?:gif|jpg|jpeg|png)$/i', $mimeType)){
		...
		file_put_contents($tempfile4, $this->filePrependSecurityBlock . $imgType2 . ' ?' . '>'); //6 extra bytes, first 3 being image type
	...
	protected function sendImageHeaders($mimeType, $dataSize){
        ...
        if(strtolower($mimeType) == 'web' || strtolower($mimeType) == 'image/web'){
            $mimeType = 'image/webp';
        }
