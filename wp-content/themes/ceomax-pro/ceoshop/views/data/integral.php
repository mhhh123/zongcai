...
// 积分类型
$opFilters = [];
foreach (CeoShopCoreUser::INTEGRAL_TYPE as $tK => $tV) {
    $opFilters[] = ['text' => $tV, 'value' => $tK];
}
$opFilters = json_encode($opFilters);
$opFormatText = json_encode(CeoShopCoreUser::INTEGRAL_TYPE);

?>
<?php ceo_shop_echo_admin_loading() ?>
<div class="wrap">
    <div id="ceo-app">
        <h1 class="wp-heading-inline">积分记录</h1>
        ...
        <el-row>
            <el-col>
                <el-table ref="indexTable" v-loading="loading" :data="tableData" @sort-change="handleSortChange" @filter-change="filterChange" stripe style="width: 100%">
                    ...
                    <el-table-column prop="num" label="积分变更">
                    ...
                    <el-table-column prop="after" label="积分"></el-table-column>
                    <el-table-column prop="type" label="积分类型" column-key="type" :filters="opFilters">
                        <template slot-scope="scope">
                            <span>{{ opFormatText[scope.row.type] }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column prop="op" label="积分状态" column-key="op" :filters="[{text: '增加', value: 1}, {text: '减少', value: 2}]" :filter-multiple="false">
                        <template slot-scope="scope">
    ...
    new Vue({
        ...
        methods: {
            load() {
                this.loading = true
                let _this = this
                axios.get('/wp-json/ceo-shop/v1/data-integral/index', {
            ...
            handleDelete(id) {
                this.loading = true
                let _this = this
                axios.post('/wp-json/ceo-shop/v1/data-integral/delete', {
                        ids: [id]
                    }, {
                        headers: {
