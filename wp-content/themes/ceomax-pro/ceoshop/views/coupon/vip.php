...
// 会员优惠券使用范围/使用等级
$vipIdOptions = CeoShopCoreUser::getVipGradeList(1);
$vipIdFormatText = CeoShopCoreUser::getVipGradeList(2);
...
<div class="wrap">
    <div id="ceo-app">
        <div v-show="!detailVisible">
            <h1 class="wp-heading-inline">会员优惠券管理</h1>
            <hr class="wp-header-end" style="margin-bottom: 8px">
            <el-row>
                <el-col :span="20">
                    <el-form :inline="true" :model="indexParams.search" size="mini">
                        <el-form-item label="会员优惠券名称">
                            <el-input v-model="indexParams.search.title" placeholder="请输入会员优惠券名称" clearable></el-input>
                        </el-form-item>
                        <el-form-item label="会员优惠券卡密">
                            <el-input v-model="indexParams.search.code" placeholder="请输入会员优惠券卡密" clearable></el-input>
                        </el-form-item>
                        <el-form-item>
                ...
                <el-col :span="4" style="text-align: right">
                    <el-button type="primary" icon="el-icon-document-add" size="mini" @click="dialogGenerateFormVisible = true">会员优惠券生成
                    </el-button>
                    <!-- <el-button type="primary" icon="el-icon-download" size="mini" @click="dialogExportFormVisible = true">会员优惠券导出
                    </el-button> -->
                    <el-button type="primary" icon="el-icon-position" size="mini" @click="dialogGiveFormVisible = true">会员优惠券赠送
                    </el-button>
                </el-col>
            </el-row>
            <el-row>
                <el-col>
                    <el-table ref="indexTable" v-loading="loading" :data="tableData" @sort-change="handleSortChange" @filter-change="filterChange" stripe style="width: 100%">
                        ...
                        <el-table-column prop="title" label="会员优惠券名称"></el-table-column>
                        <el-table-column prop="code" label="会员优惠券卡密"></el-table-column>
                        <el-table-column prop="money" label="会员优惠券面值" sortable="custom"></el-table-column>
                        <el-table-column prop="validity" label="会员优惠券有效期限" sortable="custom">
                            <template slot-scope="scope">
                                <span>{{ scope.row.validity }}天</span>
                            </template>
                        </el-table-column>
                        <el-table-column prop="allow_vips" label="会员优惠券使用范围">
                            <template slot-scope="scope">
                                <el-tag v-if="scope.row.allow_vips.length == 0">全部可用</el-tag>
                                <el-tag v-else v-for="vip in scope.row.allow_vips" style="margin-right: 4px; margin-bottom: 2px;">{{ vipIdFormatText[vip] }}
                                </el-tag>
                            </template>
                        </el-table-column>
            ...
            <el-dialog title="会员优惠券生成" width="25%" :visible.sync="dialogGenerateFormVisible">
                <el-form ref="generateForm" :model="generateForm" :rules="generateFormRules">
                    <el-form-item label="会员优惠券名称" prop="title">
                        <el-input v-model="generateForm.title" placeholder="如注册奖励会员优惠券"></el-input>
                    </el-form-item>
                    <el-form-item label="会员优惠券面值" prop="money">
                        <el-input-number :min="1" :precision="0" v-model="generateForm.money" controls-position="right" style="width: 100%" placeholder="请输入会员优惠券面值">
                        </el-input-number>
                    </el-form-item>
                    <el-form-item label="会员优惠券有效期限" prop="validity">
                    ...
                    <el-form-item label="会员优惠券使用范围" prop="allow_vips">
                        <el-select v-model="generateForm.allow_vips" multiple style="width: 100%" placeholder="请选择会员优惠券使用范围">
                            <el-option v-for="option in vipIdOptions" :key="option.value" :label="option.text" :value="option.value"></el-option>
                        </el-select>
                        <span class="form-help-text">不填写默认对全部等级生效</span>
                    </el-form-item>
                </el-form>
            ...
            <el-dialog title="会员优惠券导出" width="25%" :visible.sync="dialogExportFormVisible">
        ...
        <div v-show="detailVisible">
            <h1 class="wp-heading-inline">会员优惠券详情</h1>
            ...
            <el-row>
                <el-col>
                    <el-table ref="detailTable" v-loading="detailLoading" :data="detailTableDate" @sort-change="handleDetailSortChange" @filter-change="filterDetailChange" stripe style="width: 100%">
                        ...
                        <el-table-column prop="user_vip_id" label="使用等级" column-key="use_vip_id" :filters="vipIdOptions">
                            <template slot-scope="scope">
                                <span v-if="scope.row.status == 1">{{ vipIdFormatText[scope.row.use_vip_id] }}</span>
                                <span v-else>-</span>
                            </template>
                        </el-table-column>
                        ...
                        <el-table-column prop="created_time" label="领取时间"></el-table-column>
                        <!-- <el-table-column width="100" label="操作">
                            <template slot-scope="scope">
...
<script>
    new Vue({
        ...
        data: {
            ...
            vipIdOptions: <?php echo $vipIdOptions ?>,
            vipIdFormatText: <?php echo $vipIdFormatText ?>,
        ...
        methods: {
            load() {
                this.loading = true
                let _this = this
                axios.get('/wp-json/ceo-shop/v1/coupon-vip/index', {
            ...
            onCodeCreate() {
                ...
                let _this = this
                axios.post(
                    '/wp-json/ceo-shop/v1/coupon-vip/create',
                    this.generateForm, {
                        headers: {
            ...
            onCodeExport() {
                ...
                let url = '/wp-json/ceo-shop/v1/coupon-vip/export?status=' + this.exportForm.status
                link.href = url;
                link.setAttribute('download', '会员优惠券导出.csv');
                link.click();
            ...
            onCodeGive() {
                ...
                let _this = this
                axios.post(
                    '/wp-json/ceo-shop/v1/coupon-vip/give',
                    this.giveForm, {
                        headers: {
                            'Content-Type': 'multipart/form-data'
                        }
            ...
            onDetailOpen(id) {
                ...
                this.detailParams.search.coupon_vip_id = id
                this.onDetailReset()
            },
            onDetailClose() {
            ...
            detailLoad() {
                ...
                let _this = this
                axios.get('/wp-json/ceo-shop/v1/coupon-vip/detail-index', {
            ...
            onDetailReset() {
                let temp_coupon_vip_id = this.detailParams.search.coupon_vip_id
                this.detailParams = {
                    page: 1,
                    pageSize: 10,
                    sort: {},
                    ...
                    search: {
                        display_name: '',
                        coupon_vip_id: temp_coupon_vip_id,
                    },
                }
            ...
            handleDelete(id) {
                this.loading = true
                let _this = this
                axios.post('/wp-json/ceo-shop/v1/coupon-vip/delete', {
                        ids: [id]
                    }, {
                        headers: {
                            'Content-Type': 'multipart/form-data'
            ...
            handleDetailDelete(id) {
                this.loading = true
                let _this = this
                axios.post('/wp-json/ceo-shop/v1/coupon-vip/detail-delete', {
                        ids: [id]
                    }, {
                        headers: {
                            'Content-Type': 'multipart/form-data'
