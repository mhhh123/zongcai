 ...
 * ------------------------------------------------------------------------------
 */
class CeoShopPaymentEpayAlipay implements CeoShopPaymentInterface
{
    // 支付渠道类型标识，不可修改
    const TYPE = 16;
    ...
    public function verify()
    {
        ...
        if ($sign !== $_GET['sign']) {
            throw new \Exception('epay_alipay支付失败：Invalid sign!');
        }
        ...
        if ($res['trade_status'] == 'TRADE_SUCCESS') {
            ...
            throw new \Exception('epay_alipay支付失败：' . json_encode($res));
        }
    }
    ...
    private function submit($order, $mchid, $mchsecret, $url)
    {
        $data = [
            ...
            'notify_url' => home_url('ceo-shop-pay-notify/' . $this->orderType . '/epay_alipay/'), // 异步通知地址	
