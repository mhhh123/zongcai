...
$pageSize = 10;
$result = CeoShopMemberPromotion::getRelationCommissionList($userId, $page, $pageSize);
...
<div class="member-promotion">
    ...
    <div class="bottom ceo-background-default ceo-margin-bottom">
        <div class="title">
            ...
            <a href="/member/commission" class="current">推广佣金</a>
        </div>
        <?php if (!empty($result['data'])) : ?>
            <div class="commission">
                <div class="header">
                    <div class="header-1">用户</div>
                    <div class="header-2">类型</div>
                    <div class="header-3">关系</div>
                    <div class="header-4">消费</div>
                    <div class="header-5">佣金</div>
                    <div class="header-6">时间</div>
                </div>
                <ul class="list">
                    <?php foreach ($result['data'] as $row) : ?>
                        <li>
                            <div class="list-1"><?php echo $row->display_name ?></div>
                            <div class="list-2"><?php echo CeoShopCoreProduct::PRODUCT_NAME[$row->product_type] ?></div>
                            <?php $pidInfo = CeoShopMemberPromotion::getParentRelation($row->buyer_user_id) ?>
                            <div class="list-3"><?php if ($pidInfo->pid_1 == $userId) {
                                                    echo '一级推广';
                                                } elseif ($pidInfo->pid_2 == $userId) {
                                                    echo '二级推广';
                                                } elseif ($pidInfo->pid_3 == $userId) {
                                                    echo '三级推广';
                                                } ?></div>
                            <div class="list-4"><?php echo $row->actual_money ?></div>
                            <div class="list-5"><?php echo $row->commission ?></div>
                            <div class="list-6"><?php echo $row->created_time ?></div>
                        </li>
                    <?php endforeach; ?>
                </ul>
                <div class="paginate">
